...
4.  **Refactor `PitchAnalysisEngine`:**
    -   Update `Analyze()` to use the new algorithm.

## Acceptance Criteria

1.  The detected pitch is significantly more stable than the ZCR estimator.
2.  The engine correctly identifies pitches for sustained vocal-like tones.
3.  Performance is maintained for real-time operation.
