// Converted from tmp/k/src/utils/qstringutils.h
...
public:
    ...
    /** @returns a file name based on @param name that is unique in @param names.
     *  This is achieved by adding a number as suffix before the extension to the @param name in case name
     *  does already exist in @param names.
     */
    static QString getUniqueFileName(const QStringList &names, const QString &name);
