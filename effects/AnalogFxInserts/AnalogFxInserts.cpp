...
AnalogFxInsertsGraph::AnalogFxInsertsGraph() {
...
void AnalogFxInsertsGraph::InitializeGraph() {
...
void AnalogFxInsertsGraph::UpdateGraph() {
	// Update graph based on current parameter state
	// This allows visualization to react to parameter changes
}

const UI::GraphVisualizationCtrl& AnalogFxInsertsGraph::GetVisualizationCtrl() const {
	return viz_ctrl;
}
...
void AnalogFxInserts::InitializeGraph() {
...
void AnalogFxInserts::ProcessGraph(PluginSDK::ProcessContext& ctx) {
...
void AnalogFxInserts::ProcessNode(const GraphNode& node, PluginSDK::ProcessContext& ctx) {
...
void AnalogFxInserts::SetParameter(int index, double value) {
	if (index >= 0 && index < parameters.GetCount()) {
...
void AnalogFxInserts::UpdateParameter(int index, double value) {
...
String AnalogFxInserts::GetParameterName(int index) const {
...
String AnalogFxInserts::GetParameterText(int index) const {
...
void AnalogFxInserts::UpdateVisualization() {
	// Update the visualization to reflect current state
	// This sends current graph state to the UI visualization
}

void AnalogFxInserts::ProcessMidiEvents(const Upp::Vector<PluginSDK::MidiEvent>& events) {
...
void AnalogFxInserts::RouteMidiEvent(const PluginSDK::MidiEvent& evt) {
