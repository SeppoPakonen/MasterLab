...
RestorationNoiseSuiteGraph::RestorationNoiseSuiteGraph() {
...
void RestorationNoiseSuiteGraph::InitializeGraph() {
...
void RestorationNoiseSuiteGraph::UpdateGraph() {
	// Update graph based on current parameter state
	// This allows visualization to react to parameter changes
}

const UI::GraphVisualizationCtrl& RestorationNoiseSuiteGraph::GetVisualizationCtrl() const {
	return viz_ctrl;
}
...
void RestorationNoiseSuite::InitializeGraph() {
...
void RestorationNoiseSuite::ProcessGraph(PluginSDK::ProcessContext& ctx) {
...
void RestorationNoiseSuite::ProcessNode(const GraphNode& node, PluginSDK::ProcessContext& ctx) {
...
void RestorationNoiseSuite::UpdateGraphRouting() {
	// Update routing based on current graph configuration
	// This connects the graph nodes according to their inputs/outputs
}

// Parameter Access Methods
int RestorationNoiseSuite::GetNumParameters() const {
	return kParameterCount;
}

double RestorationNoiseSuite::GetParameter(int index) const {
...
void RestorationNoiseSuite::SetParameter(int index, double value) {
	if (index >= 0 && index < parameters.GetCount()) {
...
void RestorationNoiseSuite::UpdateParameter(int index, double value) {
...
String RestorationNoiseSuite::GetParameterName(int index) const {
...
String RestorationNoiseSuite::GetParameterText(int index) const {
...
void RestorationNoiseSuite::UpdateVisualization() {
	// Update the visualization to reflect current state
	// This sends current graph state to the UI visualization
}

void RestorationNoiseSuite::ProcessMidiEvents(const Upp::Vector<PluginSDK::MidiEvent>& events) {
...
void RestorationNoiseSuite::RouteMidiEvent(const PluginSDK::MidiEvent& evt) {
