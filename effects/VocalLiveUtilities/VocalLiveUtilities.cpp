...
VocalLiveUtilitiesGraph::VocalLiveUtilitiesGraph() {
...
void VocalLiveUtilitiesGraph::InitializeGraph() {
...
void VocalLiveUtilitiesGraph::UpdateGraph() {
	// Update graph based on current parameter state
	// This allows visualization to react to parameter changes
}

const UI::GraphVisualizationCtrl& VocalLiveUtilitiesGraph::GetVisualizationCtrl() const {
	return viz_ctrl;
}
...
void VocalLiveUtilities::InitializeGraph() {
...
void VocalLiveUtilities::ProcessGraph(PluginSDK::ProcessContext& ctx) {
...
void VocalLiveUtilities::ProcessNode(const GraphNode& node, PluginSDK::ProcessContext& ctx) {
...
void VocalLiveUtilities::UpdateGraphRouting() {
	// Update routing based on current graph configuration
	// This connects the graph nodes according to their inputs/outputs
}

// Parameter Access Methods
int VocalLiveUtilities::GetNumParameters() const {
	return kParameterCount;
}

double VocalLiveUtilities::GetParameter(int index) const {
...
void VocalLiveUtilities::SetParameter(int index, double value) {
	if (index >= 0 && index < parameters.GetCount()) {
...
void VocalLiveUtilities::UpdateParameter(int index, double value) {
...
String VocalLiveUtilities::GetParameterName(int index) const {
...
String VocalLiveUtilities::GetParameterText(int index) const {
...
void VocalLiveUtilities::UpdateVisualization() {
	// Update the visualization to reflect current state
	// This sends current graph state to the UI visualization
}

void VocalLiveUtilities::ProcessMidiEvents(const Upp::Vector<PluginSDK::MidiEvent>& events) {
...
void VocalLiveUtilities::RouteMidiEvent(const PluginSDK::MidiEvent& evt) {
